...
def _mvp1_compact_marker_visible(page) -> bool:
    """MVP1 compact digest markers — avoid false positives from collapsed Screen map caption."""
    for marker in (
        "MVP1 data quality",
        "MVP1 primary output",
        "MVP1 output:",
...
def _scroll_to_mvp1_compact_block(page) -> None:
    """Scroll until compact MVP1 digest markers are attached."""
    for _ in range(8):
...
def _collect_verification_observation(page, result: ScenarioResult) -> None:
    try:
        if result.scenario == "MVP1_compact_verification":
            _scroll_to_mvp1_compact_block(page)
        markers = (
            "MVP1 output:",
...
def _collect_feedback_panel_observation(page, result: ScenarioResult) -> None:
    """Retired: friends-first feedback panel no longer ships on public demo."""
