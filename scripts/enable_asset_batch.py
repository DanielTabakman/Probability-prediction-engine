...
def _run_witness(asset_ids: list[str], *, live: bool) -> tuple[bool, dict]:
    cmd = [
        sys.executable,
        str(_REPO_ROOT / "scripts" / "witness_asset_catalog.py"),
        "--json",
    ]
    for aid in asset_ids:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if to_enable and not args.skip_witness and (args.apply or args.dry_run):
        witness_ok, witness_report = _run_witness(to_enable, live=bool(args.live_witness))
