...
def _rollup_week_seconds(
    repo: Path,
    week_monday: str,
    *,
    sends: list[dict[str, Any]] | None = None,
) -> tuple[int, int, int] | None:
    """Return (runningSec, downSec, downUnexpectedSec) for Mon–Sun of week_monday."""
    try:
        start = datetime.fromisoformat(week_monday).date()
    except ValueError:
        return None
    running = down = unexpected = 0
    for offset in range(7):
        day = (start + timedelta(days=offset)).isoformat()
        m = get_day_metrics(repo, day, sends=sends)
        running += int(m.get("runningSec") or 0)
        down += int(m.get("downSec") or 0)
        unexpected += int(m.get("downUnexpectedSec") or 0)
    if running + down <= 0:
        return None
    return running, down, unexpected


def week_uptime_summary_line(
    repo: Path,
    week_monday: str,
    *,
    sends: list[dict[str, Any]] | None = None,
) -> str | None:
    """One-line loop uptime for weekly phone digest (this week vs prior week)."""
    this = _rollup_week_seconds(repo, week_monday, sends=sends)
    if this is None:
        return None
    running, down, unexpected = this
    total = running + down
    pct = format_pct(running, total)
    try:
        prior_monday = (datetime.fromisoformat(week_monday).date() - timedelta(days=7)).isoformat()
    except ValueError:
        prior_monday = ""
    prior = _rollup_week_seconds(repo, prior_monday, sends=sends) if prior_monday else None
    line = f"Loop uptime: {pct} ({format_duration(running)} up, gap {format_duration(unexpected)})"
    if prior:
        p_run, p_down, _ = prior
        p_total = p_run + p_down
        if p_total > 0:
            delta = int(round(100 * running / total - 100 * p_run / p_total))
            line += f" — {delta:+d} pt vs prior week"
    return line
