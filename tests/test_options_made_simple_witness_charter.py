...
RB12_PLAN = "docs/SOP/PHASE_PLANS/msos_session_resume_v1_relay.json"
...
def test_order_12_queue_row_is_done_after_closeout() -> None:
    queue = _json("docs/SOP/PHASE_QUEUE.json")
    closed = next(item for item in queue["items"] if item.get("planPath") == RB12_PLAN)
    assert closed["status"] == "DONE"
