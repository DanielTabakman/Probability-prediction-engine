...
def test_msos_ranking_lib_declares_no_advice_contract_and_weights() -> None:
    ...
    assert "buildCandidatesFromExposureMenu" in lib
    assert "buildCandidateFromStrategySuggestion" in lib
    assert "optionsHorizonComparison" not in lib
    assert "options_horizon_comparison" not in lib
...
def test_expression_planning_panel_wires_ranking_without_advice_copy() -> None:
    ...
    assert "buildCandidatesFromExposureMenu" in panel
