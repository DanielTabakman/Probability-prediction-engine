...
class TestDecisionReadyReviewPayload(unittest.TestCase):
    ...
    def test_mvp1_excludes_trade_ticket_language(self) -> None:
        ...
        self.assertIn("candidate hypotheses", p["linkage_line"])
        self.assertNotIn("Trade ticket (copy/paste)", p["bullets"][-1])
        assert_no_advisory_language(_join_payload_text(p))

    def test_mvp1_hypothesis_framing_in_structure_line(self) -> None:
        p = build_decision_ready_review_payload(
            self._minimal_applicable_verification(glance={"fit_note": "Fit is not recommendation."}),
            mvp1_exclude_execution_ui=True,
        )
        assert p is not None
        self.assertIn("hypothesis to inspect", p["structure_line"].lower())
        self.assertIn("ranked", p["structure_line"].lower())
        assert_no_advisory_language(_join_payload_text(p))
