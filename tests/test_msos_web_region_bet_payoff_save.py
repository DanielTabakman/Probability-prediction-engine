...
def test_guided_shell_requires_explicit_payoff_confirmation_for_activation() -> None:
    ...
    guided_lib = _read("src/lib/regionBetGuidedShell.ts")
    ...
    assert "frozen_entry_snapshot: seed?.frozen_entry_snapshot" in guided_lib
    assert "isRegionBetGuidedShellFrozen" in guided_lib
...
def test_restore_contract_accepts_frozen_snapshot_and_preserves_owner_scope() -> None:
    ...
    assert "restoreRegionBetFrozenEntrySnapshot" in payoff
