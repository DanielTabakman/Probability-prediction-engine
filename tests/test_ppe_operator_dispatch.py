...
def test_resolve_dispatch_command_desktop_continue(tmp_path: Path) -> None:
    from scripts.ppe_operator_dispatch import resolve_dispatch_command

    cmd = resolve_dispatch_command(tmp_path, "DESKTOP_CONTINUE.cmd --no-pause")
    assert cmd == "DESKTOP_CONTINUE.cmd --no-pause"


def test_resolve_dispatch_command_factory_throughput(tmp_path: Path) -> None:
    from scripts.ppe_operator_dispatch import resolve_dispatch_command

    cmd = resolve_dispatch_command(tmp_path, "factory_throughput")
    assert "ppe_factory_throughput" in (cmd or "")


def test_dispatch_dry_run(tmp_path: Path) -> None:
    report = dispatch_direct_action(tmp_path, "resolve_lease", dry_run=True)
    assert report.get("dry_run") is True
    assert report.get("ok") is True
    assert report.get("steps") == []


def test_automation_preflight_blocks_relay() -> None:
    from scripts.ppe_operator_dispatch import automation_preflight_blocked

    blocked, reason, preferred = automation_preflight_blocked(
        {"branch_preflight": {"blocks_relay": True}},
        action="DESKTOP_CONTINUE.cmd --no-pause",
    )
    assert blocked is True
    assert preferred == "branch_recovery"
    assert "branch_preflight" in reason


def test_dispatch_desktop_continue_skipped_when_preflight_blocked(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("PPE_AUTO_DISPATCH", "1")

    class _Status:
        @staticmethod
        def prepare_operator_status(_repo: Path) -> dict:
            return {"branch_preflight": {"blocks_relay": True}}

    monkeypatch.setattr("scripts.ppe_operator_status.prepare_operator_status", _Status.prepare_operator_status)
