...
def test_week_uptime_summary_line(tmp_path):
    from scripts.ppe_operator_daily_metrics import record_watch_sample, week_uptime_summary_line

    tz = ZoneInfo("America/New_York")
    mon = datetime(2026, 6, 1, 10, 0, tzinfo=tz).astimezone(timezone.utc)
    tue = mon + timedelta(hours=20)
    record_watch_sample(tmp_path, loop_running=True, verdict="RUN_AUTO", now=mon)
    record_watch_sample(tmp_path, loop_running=True, verdict="RUN_AUTO", now=tue)

    line = week_uptime_summary_line(tmp_path, "2026-06-01")
    assert line is not None
    assert "Loop uptime:" in line
