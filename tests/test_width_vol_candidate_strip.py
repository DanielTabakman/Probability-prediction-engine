...
class TestWidthVolCandidateStrip(unittest.TestCase):
    ...
    def test_mvp1_snapshot_line_when_decision_present(self) -> None:
        pl = belief_disagreement_hints_payload(
            center_usd=100_000.0,
            market_peak=100_000.0,
            sigma_user=0.15,
        ...
        v = {
            ...
            "belief_disagreement": pl["belief_disagreement"],
            "density": {"market_implied": {"breeden_litzenberger": "computed"}},
            "mvp1_decision": {"primary_output_state": "candidate", "data_quality": "usable"},
        }
        out = build_width_vol_candidate_strip_payload(v)
        self.assertIsNotNone(out)
        assert out is not None
        snap = str(out.get("mvp1_snapshot_md") or "")
        self.assertIn("mvp1 snapshot", snap.lower())
        self.assertIn("candidate", snap.lower())
        self.assertIn("usable", snap.lower())
