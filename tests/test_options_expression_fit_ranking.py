...
def test_score_components_and_top_fit_are_deterministic() -> None:
    ...
    assert "financial advice" in first["limitation"]
    assert "expected-profit" in first["limitation"]
...
def test_engine_and_cli_stay_independent_of_job_a() -> None:
    from pathlib import Path

    from src.engine import options_expression_fit_ranking as engine

    repo = Path(__file__).resolve().parents[1]
    engine_src = inspect.getsource(engine)
    cli_src = (repo / "scripts" / "rank_options_expression_fit.py").read_text(encoding="utf-8")
    assert "options_horizon_comparison" not in engine_src
    assert "options_horizon_comparison" not in cli_src
    assert engine.SCORE_WEIGHTS["direction_fit"] == 30
    assert sum(engine.SCORE_WEIGHTS.values()) == 100


def test_offline_exposure_fixture_ranks_without_live_or_job_a() -> None:
    from src.viz.options_expression_fit_ranking_boundary import (
        build_options_expression_fit_ranking_response,
    )

    payload = build_options_expression_fit_ranking_response(
        {
            "QUERY_STRING": (
                "asset=NVDA&direction=long&horizon=3m&offline=1"
                "&target_horizon_days=90&max_loss_usd=800&payoff_preference=defined_risk"
            )
        }
    )
    assert payload["kind"] == "options_expression_fit_ranking"
    assert payload["source_kinds"]["exposure_menu"] == "exposure_paths"
    assert payload["source_kinds"]["strategy_suggestion"] is None
    assert payload["top_fit"]["candidate_id"].startswith("exposure:")
    assert payload["ranked"]
    assert payload["limitation"] == LIMITATION_TEXT
