...
REGION_BET_10_ID = "region_bet_payoff_save_v1"
...
def test_issue_5374_reconciled_items_do_not_reappear_after_options_made_simple_selection(monkeypatch) -> None:
    ...
    assert ready_ids == {REGION_BET_10_ID}
...
def test_selection_context_without_exclusions_preserves_region_bet_recommendation(monkeypatch) -> None:
    ...
    assert [item["work_item_id"] for item in ppe["ready_work"]] == [REGION_BET_10_ID]
    assert snapshot["recommended_next_action"]["work_item_id"] == REGION_BET_10_ID
...
def test_selection_context_excluding_merged_options_a_is_unmatched_and_recommends_region_bet(monkeypatch) -> None:
    ...
    assert ready_ids == [REGION_BET_10_ID]
    assert rec["work_item_id"] == REGION_BET_10_ID
...
def test_selection_context_deduplicates_and_sorts_exclusions(monkeypatch) -> None:
    ...
    assert first["recommended_next_action"]["work_item_id"] == REGION_BET_10_ID
...
def test_cli_exclusion_contract_is_json_and_read_only(monkeypatch) -> None:
    ...
    assert before == after
    assert payload["recommended_next_action"]["work_item_id"] == REGION_BET_10_ID
