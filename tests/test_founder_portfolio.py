...
def test_issue_5374_reconciled_items_do_not_reappear_after_options_made_simple_selection(monkeypatch) -> None:
    ...
    assert snapshot["recommended_next_action"].get("work_item_id") not in reconciled_ids
    assert snapshot["recommended_next_action"].get("work_item_id") != REGION_BET_13_ID
...
def test_selection_context_without_exclusions_preserves_region_bet_recommendation(monkeypatch) -> None:
    ...
    assert snapshot["recommended_next_action"].get("work_item_id") != REGION_BET_13_ID
    assert "selection_explanation" not in snapshot["recommended_next_action"]
...
def test_selection_context_excluding_merged_options_a_is_unmatched_and_recommends_region_bet(monkeypatch) -> None:
    ...
    assert ready_ids == []
    assert rec.get("work_item_id") != REGION_BET_13_ID
    assert rec["selection_context"] == snapshot["selection_context"]
    assert "selection_explanation" not in rec
...
def test_selection_context_deduplicates_and_sorts_exclusions(monkeypatch) -> None:
    ...
    assert first["recommended_next_action"].get("work_item_id") != REGION_BET_13_ID
...
def test_cli_exclusion_contract_is_json_and_read_only(monkeypatch) -> None:
    ...
    assert before == after
    assert payload["recommended_next_action"].get("work_item_id") != REGION_BET_13_ID
