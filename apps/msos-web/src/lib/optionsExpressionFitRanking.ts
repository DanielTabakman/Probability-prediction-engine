...
const payoffLens: Record<PayoffPreference, FitLensId[]> = {
...
const HORIZON_CHIP_DAYS: Record<Exclude<HorizonChip, "any">, number> = {
  "3m": 90,
  "12m": 365,
};

export function horizonChipToDays(horizon: HorizonChip | null | undefined): number | undefined {
  if (!horizon || horizon === "any") return undefined;
  return HORIZON_CHIP_DAYS[horizon];
}
...
export function buildCandidatesFromExposureMenu(
  payload: ExposureMenuPayload | null | undefined,
): ExpressionFitCandidate[] {
  if (!payload?.paths?.length) return [];
  const horizonDays = horizonChipToDays(payload.horizon);
  return payload.paths.map((path, index) => {
    const { legs, ...rest } = path;
    return {
      ...rest,
      candidate_id: `exposure:${path.path_id}`,
      source: "exposure_menu" as const,
      source_order: index,
      horizon_days: path.time_bound === "dated" ? horizonDays : undefined,
      max_loss_usd: path.cost_hint_usd ?? null,
      legs: legs?.map((leg) => ({
        side: leg.side ?? "",
...
export function scoreExpressionCandidate(
  ...
  const lenses = new Set<FitLensId>(candidate.fit_lenses ?? []);
