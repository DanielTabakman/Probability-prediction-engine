import type { RegionBetContract, RegionBetFrozenEntrySnapshot } from "@/lib/regionBet";
...
export function isRegionBetMonitorable(regionBet: Pick<RegionBetContract, "lifecycle">): boolean {
  return regionBet.lifecycle.status === "active" || regionBet.lifecycle.status === "monitoring";
}
...
): RegionBetFrozenEntrySnapshot | null {
  const expression = regionBet.selected_expression_ref;
  if (!expression?.expression_id.trim()) return null;
  return {
...
export function restoreRegionBetFrozenEntrySnapshot(
  regionBet: RegionBetContract,
): RegionBetFrozenEntrySnapshot | null {
  return regionBet.frozen_entry_snapshot ?? null;
}
