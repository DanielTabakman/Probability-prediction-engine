...
set "WORKER_MODE=acp"
if not "%PHASE_PLAN%"=="" (
  for /f "delims=" %%w in ('python "%REPO_ROOT%\scripts\resolve_slice_worker_mode.py" --repo-root "%REPO_ROOT%" --slice-id "%SLICE_ID%" --phase-plan "%PHASE_PLAN%" 2^>nul') do set "WORKER_MODE=%%w"
)
if /i "%PPE_WORKER_MODE%"=="deterministic" set "WORKER_MODE=deterministic"
if /i "%PPE_SKIP_ACP%"=="1" set "WORKER_MODE=deterministic"
...
if /i "%WORKER_MODE%"=="deterministic" (
  set "PYTHONPATH=%REPO_ROOT%"
  python "%REPO_ROOT%\scripts\ppe_slice_worker.py" --repo-root "%REPO_ROOT%" --slice-id "%SLICE_ID%" --sprint-spec "%SPRINT_SPEC%" --declared-plane "%DECLARED_PLANE%" --build-branch "%BUILD_BRANCH%" --phase-plan "%PHASE_PLAN%" --sus-minutes %SUS_MIN% --hard-minutes %HARD_MIN%
  set "EXIT_CODE=%ERRORLEVEL%"
  goto slice_after_orch
)

if /i "%WORKER_MODE%"=="local-agent" (
  set "PYTHONPATH=%REPO_ROOT%"
  python "%REPO_ROOT%\scripts\phase_orchestrator_v0.py" run-slice --slice-id "%SLICE_ID%" --sprint-spec-path "%SPRINT_SPEC%" --declared-plane "%DECLARED_PLANE%" --baseline-branch %BASELINE_BRANCH% --build-branch "%BUILD_BRANCH%" --worker-mode agent-cli --sus-minutes %SUS_MIN% --hard-minutes %HARD_MIN%
  set "EXIT_CODE=%ERRORLEVEL%"
  goto slice_after_orch
)
