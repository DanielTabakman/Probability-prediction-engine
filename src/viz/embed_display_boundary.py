...
def create_display_payload_wsgi_app(
    ...
    def app(environ: dict[str, Any], start_response: Callable[..., Any]) -> list[bytes]:
        ...
        from src.viz.implied_range_api import handle_implied_range_wsgi_path
        ...
        implied_range = handle_implied_range_wsgi_path(path, environ)
        if implied_range is not None:
            status, body = implied_range
            start_response(
                status,
                [
                    ("Content-Type", "application/json; charset=utf-8"),
