...
_HORIZON_CHIP_DAYS = {"3m": 90, "12m": 365}
...
def build_options_expression_fit_ranking_response(environ: dict[str, Any]) -> dict[str, Any]:
    ...
    chip_days = _HORIZON_CHIP_DAYS.get(horizon)
    ...
    for i, path in enumerate(exposure_payload.get("paths") or []):
        ...
        row = {
            **path,
            "candidate_id": f"exposure:{path.get('path_id')}",
            "source": "exposure_menu",
            "source_order": i,
        }
        if row.get("time_bound") == "dated" and chip_days is not None and row.get("horizon_days") is None:
            row["horizon_days"] = chip_days
