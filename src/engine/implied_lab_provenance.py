...
from src.engine.belief_disagreement_hints import build_disagreement_scan_payload
...
TRUST_STRIP_VERIFICATION_POINTER = "Full traces and numeric inputs: expand **Verification** below."
...
def _belief_vs_market_glance_block(
    *,
    ...
    gap_txt = "—"
    if belief_largest_gap_price_usd is not None and isinstance(belief_largest_gap_price_usd, (int, float)):
...
def _verification_summary_block(
    *,
    ...
    return {
        ...
        "derivation_paths": {
            "reference_density": "Lognormal on chart grid from forward + ATM IV + T (risk-neutral reference, purple).",
            "market_implied_density": "Breeden–Litzenberger from call marks when ≥3 strikes (orange curve).",
            "disagreement_category": (
                "Peak alignment × width-band thresholds → category_id; "
                "L₁ PDF gap labels the strength of shape difference, not the category."
            ),
            "cost_and_payoff": "USD leg value = mark_btc × forward; grid P&L matches the green payoff line.",
        },
        "lab_mode": lab_mode,
...
def build_belief_disagreement_verification_trace(
    ...
    return {
        ...
        "raw": {
            ...
            "contract_schema_version": belief_disagreement.get("contract_schema_version"),
        },
    }
...
def build_verification_payload(
    *,
    ...
    return {
        ...
        "belief_disagreement_verification": build_belief_disagreement_verification_trace(belief_disagreement),
        "strategy_summary": {
