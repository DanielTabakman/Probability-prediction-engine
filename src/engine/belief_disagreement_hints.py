...
def build_belief_disagreement_contract(
    *,
    ...
    if category_id == "directional":
        ...
        interp = "Your belief peak and width are close to the market reference at this horizon."
        if shape_gap_strength == "High":
            interp += " *Overall PDFs can still differ in detail — see largest gap above.*"
        summary_lines = [title, "", interp]
...
def _digest_peak_line(trace: dict[str, Any]) -> str:
    ...
    return f"Your belief peak vs reference modal: ~${center:,.0f} vs ~${mpeak:,.0f} ({ref_short})."
...
def _digest_main_line(
    *,
    ...
    if cat == "directional":
        ...
        return prefix + f"**directional ({tilt})** — peak shifted vs the reference modal while width is similar."
    if cat == "width_vol":
        wlab = (
            "wider tails / dispersion"
            if wb == "wider"
            else "narrower / contained range"
            if wb == "narrower"
            else "width differs"
        )
        return (
    ...
    if cat == "mixed":
        ...
        return prefix + f"**mixed** — a **{tilt}** peak shift **and** **{wlab}** uncertainty vs the market reference."
...
def build_disagreement_scan_payload(
    ...
    for fam in families:
        ...
        if not label:
            continue
        fit_bridge_bullets.append(f"- **{label}:** {rationale}" if rationale else f"- **{label}**")
