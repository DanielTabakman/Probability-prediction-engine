...
python -c "import json,datetime,pathlib; p=r'%ACTIVE_RUN%'; d={'kind':'phase_raw','plan_path':r'%PLAN_PATH%','baseline_branch':r'%BASELINE_BRANCH%','ts_utc':datetime.datetime.utcnow().replace(microsecond=0).isoformat()+'Z'}; pathlib.Path(p).parent.mkdir(parents=True, exist_ok=True); pathlib.Path(p).write_text(json.dumps(d,indent=2), encoding='utf-8')" >nul 2>nul
...
if "%ERRORLEVEL%"=="0" (
  ...
  if exist "%ORCH_ROOT%\\node_modules\\.bin\\tsx.cmd" (
    call "%ORCH_ROOT%\\node_modules\\.bin\\tsx.cmd" src\\cli.ts run-phase "%REPO_ROOT%" "%PLAN_PATH%" "%BASELINE_BRANCH%"
  ) else (
...
python "%REPO_ROOT%\scripts\write_last_run_report.py" --repo-root "%REPO_ROOT%" --kind phase_raw --exit-code %EXIT_CODE% --plan-path "%PLAN_PATH%" --baseline-branch "%BASELINE_BRANCH%" >nul 2>nul
