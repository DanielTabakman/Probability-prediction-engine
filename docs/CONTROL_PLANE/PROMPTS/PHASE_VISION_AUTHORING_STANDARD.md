...
Govern **how** `PHASE_VISION_CURRENT.md` should be written and maintained so it stays **phase-specific**, **manager-useful**, and **high-signal**: clear outcomes, UX/semantic priorities, explicit deferrals, and drift risks—without turning into a sprint doc.
...
- a **manager steering aid** for choosing and shaping sprints (what matters most right now)  
...
- a sprint plan, backlog, or acceptance checklist  
...
- a rewrite of `docs/VISION/VISION_MASTER.md` (enduring vision)

If you find yourself listing many tasks, PR-sized steps, or file paths, you are drifting into SOP/sprint territory.
...
6. **Phase-adjacent specs** (e.g. `docs/SPRINT_1_SPEC.md`) — use to ground **what this phase means**, but do not paste implementation rules or convert into a sprint checklist.
...
- **Phase-specific**: write for the current phase horizon; do not restate the entire master vision.  
- **Manager-friendly**: emphasize decision constraints (“what matters / what to defer / what to watch”), not step-by-step execution.  
- **Concise and high-signal**: every line should help shape sprint objectives or protect semantics/UX.  
...
- **Defer explicitly**: name out-of-scope items so sprints don’t accrete “nice-to-haves.”  
...
Do **not** update it for every sprint’s micro-detail. The doc should remain stable across multiple sprints inside the same phase.

## Drift checks

Before treating the draft as done, ask:

- Did I accidentally write a **sprint doc** (tasks, acceptance steps, file-level plans)?  
- Did I leak **implementation details** that belong in a sprint or tech note?  
...
- [ ] “Done enough” defines a **clear bar** a manager can use to accept sprint progress.  
