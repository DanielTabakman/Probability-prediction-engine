...
This standard governs **how** those windows should write or update `VISION_MASTER.md`: scope, sources, section shape, tone, when to edit, and how to self-check. Follow it so the master vision stays **durable**, **concise**, and **non-sprint**.
...
- **mostly independent** of current sprint details and current phase execution  
...
2. **`docs/SOP/ORIGINAL_SPEC.md`** — cycle-stable product target; extract only what is **long-lived**, not sprint-only scope.  
...
Use `## Notes` for boundaries (e.g. relationship to phase vision, ORIGINAL_SPEC, or where roadmap detail lives)—**not** for sprint tasks.
...
- **Do not overfit** to the current sprint or single screen.  
...
- **multiple stable docs** now imply a **better synthesis** for long-horizon intent.  
...
- Did **phase-specific** details (layout, Sprint 1 widgets, one screen) **leak** into the master?  
...
- [ ] No sprint IDs, acceptance checklists, or file-level implementation plans in the master.  
