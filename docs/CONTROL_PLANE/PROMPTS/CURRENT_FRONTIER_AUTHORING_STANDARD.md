...
Govern **how** `CURRENT_FRONTIER.md` should be written and maintained so it stays a **live, manager-friendly steering doc**: a tight statement of the current phase, what “success” means, what sprint is active, and a small set of strong next sprint candidates—without becoming a backlog dump or diary.
...
- a decision aid listing **a few larger, testable sprint options** that materially advance the phase  
- explicit about **avoid-for-now**, **risks/uncertainty**, and **stop/escalate** conditions

It should help a manager choose the next sprint and define it crisply (often via `docs/SOP/SPRINT_TEMPLATE.md`), while keeping enough context to prevent drift.
...
- a replacement for a formal sprint file (`docs/SOP/SPRINT_00X.md`)  
...
3. **`docs/SOP/ORIGINAL_SPEC.md`** — cycle anchor; ensure the frontier sprints converge to the cycle target.  
4. **`docs/SOP/HANDOFF.md`** — what was actually done, what verification exists, and what is currently fragile.  
5. **SOP execution rules** — `docs/SOP/MANAGER_LOOP.md`, `docs/SOP/WORKER_LOOP.md`, `docs/SOP/OPERATING_RULES.md`, and `docs/SOP/SPRINT_TEMPLATE.md` (to keep sprint shaping and stop/escalate rules consistent).  
6. **Specs/contracts relevant to the phase** — e.g. `docs/SPRINT_1_SPEC.md`, `docs/SEMANTIC_CONTRACTS.md`, and test/smoke docs when they affect sprint credibility.

If there is conflict between sources, prefer **reality + authoritative intent** (HANDOFF evidence + phase vision + original spec) over wishful plans.
...
6. `## Next best sprint candidates`  
7. `## Avoid for now`  
8. `## Known risks / uncertainty`  
9. `## Stop / escalate conditions`  
10. `## Last updated`  

Keep each section short; “Next best sprint candidates” should usually be **2–5 items**, not 20.
...
- **Support larger, testable sprints**: prefer a few meaningful options over micro-patches, as long as acceptance/verification is credible.  
- **Reflect current reality**: “Current sprint,” “Completed recently,” and risks must match what actually happened.  
- **Avoid generic filler**: no vague “improve performance” or “refactor code” without a testable objective.  
- **Constrain scope**: each sprint candidate should have a single coherent objective and clear boundaries (what it includes/excludes).  
...
- after each meaningful sprint outcome (completed, halted, or re-scoped), or  
- when the phase or success condition changes, or  
- when reality changes (new risks, broken verification, new constraints) that should affect sprint choice

Do **not** update it to record every tiny activity; that belongs in `HANDOFF.md` or a sprint closeout.

## Drift checks

Before treating the draft as done, ask:

- Did “Next best sprint candidates” become a **backlog** instead of a shortlist?  
- Do candidates describe **testable outcomes** with credible verification, or just “work”?  
- Is “Current sprint” accurate, and does “Completed recently” match evidence (HANDOFF/tests/smoke)?  
- Did I introduce **new product scope** not supported by phase vision or original spec?  
...
- [ ] “Next best sprint candidates” is a **short list** of **larger, testable** options (not micro-patches; not a backlog).  
