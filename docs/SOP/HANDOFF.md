...
- Current selected slice: **`Sprint004-Slice001` — Width-disagreement candidate strip (v0)** — **SELECTED** (steward selection recorded in `docs/SOP/SPRINT_004_PHASE_2.md` §§6–8; **not** yet built)
- Next pending execution step: **PREFLIGHT** for **`Sprint004-Slice001`** — operator/steward gate per `CODEX_AUTONOMY_V1` §5 + `RELAY_RUNTIME_V0` §8 when relay is used; pin exact targets in `SPRINT_004_PHASE_2.md` §7.2. **BUILD does not start** until preflight passes and a BUILD-class pass is explicitly authorized. **Authority:** **steward selects**; **relay helps** only inside bounded **BUILD** loops after authorization. **Sprint 003** remains **evidence-plane only** (`docs/SOP/SPRINT_003_PHASE_2.md`). **No BUILD** on the sprint-opening pass. Context-budget advisory (`docs/SOP/WORKFLOW_CONTEXT_AUDIT_001.md`, `.cursor/rules/context-budget.mdc`) remains available for pass sizing.
...
**Execution posture (Sprint 004 — Phase 2 Product — CHARTERED 2026-04-21; `Sprint004-Slice001` SELECTED; Sprint 003 evidence-plane-only posture unchanged):**
- **Sprint 004 (Phase 2) — Product: Candidate Edge v1:** `docs/SOP/SPRINT_004_PHASE_2.md` — BTC-first mispricing discovery wedge; **active product execution boundary**. **Steward-selected slice:** **`Sprint004-Slice001` — Width-disagreement candidate strip (v0)** — **SELECTED**; **next** = **PREFLIGHT** for Slice001 (then authorized **BUILD**). **Steward selects**; **relay helps** only inside bounded **BUILD** loops after PREFLIGHT + explicit BUILD authorization. **This control-plane sprint-open pass did not run BUILD.**
...
- **Next execution step:** **PREFLIGHT** for **`Sprint004-Slice001`** — **BUILD blocked** until preflight passes and BUILD is explicitly authorized. **Sprint 003** remains available for future **evidence-plane** SELECTION only under its own charter; it is **not** the active product boundary.
