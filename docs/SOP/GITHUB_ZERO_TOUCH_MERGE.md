...
This enables **Allow auto-merge** and creates or updates a ruleset named **`PPE zero-touch main`** for `refs/heads/main` (PR required with **0** approvals, required checks **`CI / pytest`**, **`CI / docker_entrypoint`**, and **`CI / ui_smoke_compact`**, no force-push). Re-run the script anytime to refresh that ruleset.
...
| Require status checks to pass before merging | **On** — require **`CI / pytest`**, **`CI / docker_entrypoint`**, and **`CI / ui_smoke_compact`** (workflow **CI**, jobs **pytest**, **docker_entrypoint**, **ui_smoke_compact**). |
...
3. **`CI / pytest`**, **`CI / docker_entrypoint`**, and **`CI / ui_smoke_compact`** run; when green, GitHub merges without further human action.
...
| PR stuck “waiting on checks” | Confirm `ci.yml` is on `main` and required check names match **`CI / pytest`**, **`CI / docker_entrypoint`**, and **`CI / ui_smoke_compact`**. |
...
- [README](../../README.md) — commit/merge test gates; CI runs **ruff + full pytest** (`CI / pytest`), **Docker entrypoint smoke** (`CI / docker_entrypoint`), and **compact UI smoke** (`CI / ui_smoke_compact`).
