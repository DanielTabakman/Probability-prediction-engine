...
**Zero-touch merge policy (when tests pass):** do not use a standing human merge approval for routine work. Open a PR to **`main`**, enable **auto-merge**, and let GitHub merge when required checks (notably **`CI / pytest`**) pass. Full checklist and branch-protection settings: [GITHUB_ZERO_TOUCH_MERGE.md](GITHUB_ZERO_TOUCH_MERGE.md).

1. Implement on a **feature branch**; open a PR to **`main`** when ready; enable **auto-merge** on the PR.
2. Wait for **`CI / pytest`** (and any other required checks) to pass; GitHub merges to **`main`** without a manual merge click when auto-merge is enabled.
...
5. **Post-deploy smoke (steward, optional cadence):**  
   - `https://marketstructureos.com` loads as expected.  
   - DevTools → Network: `/static/js/` loads over **https**.  
   - `https://app.marketstructureos.com` — Cloudflare Access and full app after login (see runbook).
