...
|---|----------|------|-------------------|------------|--------------|--------|------------|
| 1 | **P0** | MSOS post-mortem review loop | `msos_trader_review_loop_v1` | [`msos_trader_review_loop_v1_relay.json`](PHASE_PLANS/msos_trader_review_loop_v1_relay.json) | **READY** | `@ppe-build-worker` | Save → review → KPI update entirely in MSOS (no Streamlit for step 4) |
| 2 | **P1** | Strategy Lab distribution CSV download | `msos_strategy_lab_dist_download_v1` | [`msos_strategy_lab_dist_download_v1_relay.json`](PHASE_PLANS/msos_strategy_lab_dist_download_v1_relay.json) | **IN FLIGHT** | `@ppe-finish-worker` then `@ppe-build-worker` | Trader export habit on `/strategy-lab` |
| 3 | **P1** | Cross-module deep links | `msos_trader_workflow_horizon_nav_v1` | [`msos_trader_workflow_horizon_nav_v1_relay.json`](PHASE_PLANS/msos_trader_workflow_horizon_nav_v1_relay.json) | PLANNED | `@ppe-build-worker` after SELECTION | Exposure → Lab, Horizon region → Lab as obvious next actions |
