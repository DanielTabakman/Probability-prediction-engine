...
- **No Sprint 003 (evidence-plane) slice selected.** **Sprint003-Slice001** **CLOSED / shipped** (2026-04-21 CONTROL-CLOSEOUT; accepted baseline tip **`e044f0fe16097da32ef7e472084e266fc5405740`**; see §9 slice ledger). §6.B evidence-plane candidates remain **deferred** pending fresh pilot evidence.
- **Interleaved CONTROL-PLANE slice — `Workflow-Hardening-Slice-001` — SELECTED (2026-04-21; advisory, non-gating).** This slice lives **outside** the evidence-plane scope of Sprint 003 (per §3 rule 3 this sprint's BUILD does not write to `docs/SOP/**`); it is a short steward-driven CONTROL-PLANE interlude that cashes in the Sprint003-Slice001 timing/context audit before any further Sprint 003 evidence-plane SELECTION. **Spec anchor:** `docs/SOP/CURRENT_FRONTIER.md` "Current feature slice" (inline). **Not** routed through `RELAY_RUNTIME_V0` / `run_selected_slice_v1` (CONTROL-PLANE only). Any further Sprint 003 §6.B evidence-plane SELECTION is **deferred** behind this interlude. Ledger entry: §9.
...
- **`Workflow-Hardening-Slice-001`** — **SELECTED 2026-04-21; status: awaiting BUILD.** Declared plane: **CONTROL-PLANE** (advisory, non-gating). **Interleaved outside Sprint 003's evidence-plane scope** — ledgered here only as a cross-reference so the sprint state remains legible; the slice itself is **not** governed by Sprint 003 §§3, 7, or 8 (those apply to evidence-plane slices routed through `RELAY_RUNTIME_V0`).
  - **Canonical spec:** `docs/SOP/CURRENT_FRONTIER.md` "Current feature slice" (inline).
  - **Grounding:** cashes in the Sprint003-Slice001 post-BUILD workflow audit (governance/context overhead near ceiling).
  - **BUILD scope (three outputs, CONTROL-PLANE only):** new `docs/SOP/WORKFLOW_CONTEXT_AUDIT_001.md` (canonical timing/context audit + NORMAL / WATCH / ESCALATE threshold bands + advisory actions); new `.cursor/rules/context-budget.mdc` (advisory Cursor rule); optional ≤ ~20-line LOAD-ALWAYS / LOAD-ON-DEMAND subsection in `docs/SOP/FRONTIER_STEWARD_PROTOCOL.md` (skip if it would enlarge the diff).
  - **Hard out-of-scope:** `CODEX_AUTONOMY_V1.md`, `JOB_REGISTRY_V1.md`, `RELAY_RUNTIME_V0.md`, this Sprint 003 spec (beyond this cross-reference entry), `src/**`, `scripts/**`, `tests/**`, `orchestrator/`, new gates, new enums, Sprint003-Slice002 / Sprint003-Slice003 selection.
  - **Execution:** short-lived branch `build/workflow-hardening-slice-001` off `recovery/frontier-steward-v2_1-baseline @ e044f0fe16097da32ef7e472084e266fc5405740`; steward-driven CONTROL-PLANE BUILD (not routed through `run_selected_slice_v1`); CONTROL-CLOSEOUT steward-only.
  - **Validation posture:** `python -m pytest -q` must stay at **117** passed (no regressions — the slice touches no runtime code). Optional post-BUILD witness: re-invoke `control_plane_consistency_check` and confirm `passed: true, findings: []` (no broken references introduced by the new canonical docs).
