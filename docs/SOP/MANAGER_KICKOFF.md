...
Single-use orientation for the agent acting as **manager** in this repo. You are **not** executing feature slices yourself unless you choose to; your job is to steer, document, review evidence, and delegate when appropriate.
...
- **Feature slice creation/approval** — name the feature slice, scope, acceptance, test plan (`SPRINT_TEMPLATE.md`)
...
You **do not** need to personally implement the feature slice. Delegating execution to a worker is appropriate when the feature slice is clear and acceptance is defined.
...
Use `docs/SPRINT_1_SPEC.md`, `docs/SEMANTIC_CONTRACTS.md`, and `docs/IMPLIED_LAB_SMOKE.md` when the feature slice touches the implied lab or copy/semantics.
...
2. Determine whether a feature slice is **already active**: an existing `docs/SOP/SPRINT_*.md` **other than** `SPRINT_TEMPLATE.md`, and/or `HANDOFF.md` filled with a real feature slice ID/title (not the placeholder line).
3. If **no** feature slice is active, pick the best next feature slice from `CURRENT_FRONTIER.md` (see **Current feature slice options** below).
4. Create `docs/SOP/SPRINT_001.md` using `SPRINT_TEMPLATE.md` — or the **next free** `SPRINT_00X` ID if `001` already exists.
5. Delegate execution to a worker when appropriate; paste or point to the feature slice spec file and `HANDOFF.md` test commands.
6. On return, **require real evidence** before updating the frontier or starting the next feature slice (see **Evidence required**).

## Delegation rule

- Delegate **implementation** when the feature slice objective, scope boundaries, and test plan are clear.
...
- Workers must **not** chain into the next feature slice without your explicit next feature slice (`WORKER_LOOP.md`).
...
- the work starts needing **structural rewrite** behavior beyond the feature slice  
- **repo hygiene** is degrading (duplicate truth, incoherent patterns)  
- convergence to `ORIGINAL_SPEC.md` is **no longer clear**  
- **multiple** next feature slices are similarly plausible and **direction is uncertain**
...
- **No feature slice file** exists under `docs/SOP/` yet except `SPRINT_TEMPLATE.md`. **`HANDOFF.md`** still has a placeholder under **Active feature slice**; **`CURRENT_FRONTIER.md`** states **no feature slice active** — treat as **no formal feature slice** until you create `SPRINT_001.md` (or next ID) and align handoff.  
...
## Current feature slice options

Derived from `CURRENT_FRONTIER.md` **Next best feature slice candidates** (reframed as risk levels).

### Bold option

| Field | Content |
|--------|---------|
| **Title** | One-screen lab closure: layout + centralized lab state |
| **Objective** | Deliver frontier candidates **1 + 2** in one feature slice: one-screen lab layout & summary closure **and** a single session/state layer driving chart, summary, and controls (`docs/SPRINT_1_SPEC.md` layout + technical principle). |
...
| Field | Content |
|--------|---------|
| **Title** | One-screen lab layout & summary closure (no state centralization in the same slice) |
| **Objective** | Frontier candidate **1 only**: chart near top, two-column layout, summary fields, expanders for advanced math — **without** taking on the full “one state layer” refactor in the same feature slice. |
| **Why it matters** | Highly visible progress toward `SPRINT_1_SPEC.md` with smaller session-state risk than the bold combo. |
...
| **Main risk** | May leave duplicate/widget-driven logic in place — acceptable if explicitly **scope-out** centralization for this feature slice; follow-up feature slice may still be needed for candidate 2. |

## Manager instruction block

1. Read the control docs (source-of-truth list).  
2. Confirm feature slice status; if none, **pick** bold, medium, or safe (or a clearly scoped subset) and **create** `docs/SOP/SPRINT_001.md` from `SPRINT_TEMPLATE.md`.  
...
6. **Continue** with the next feature slice, **request fixes**, or **stop/escalate** per the rules above.
